...
class TCPDF {
    ...
    public function Header() {
        if ($this->header_xobjid === false) {
			...
			$this->SetFont($headerfont[0], 'B', $headerfont[2] + 1);
			$this->SetX($header_x);
			// $this->Cell(175, $cell_height, $headerdata['title'], 0, 1, '', 0, '', 0);
            $this->Ln(10);
            $this->SetTextColor(0, 0, 0); 
            $this->Cell(210, 15, $headerdata['title'], 0, 1, 'C', 0, '', 0, false, 'M', 'M');
            $this->SetFont('helvetica', 'B', 10);
            $this->Cell(362, 15, $headerdata['string'], 0, 1, 'C', 0, '', 0, false, 'M', 'M');
			// header string
			$this->SetFont($headerfont[0], $headerfont[1], $headerfont[2]);
			$this->SetX($header_x);
			// $this->MultiCell(145, 15, $headerdata['string'], 0, '', 0, 1, 'C', '', true, 0, false, true, 0, 'M', 'M');
			// print an ending header line
			$this->SetLineStyle(array('width' => 0.85 / $this->k, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => $headerdata['line_color']));
			$this->SetY((2.835 / $this->k) + max($imgy, $this->y));
			if ($this->rtl) {
    ...
    public function Footer() {
        ...
        $this->Ln();
        $this->Cell(0, 0, 'P.O. Box 2295 Ruwi, Postal Code 112, Sultanate of Oman, Tel. : 24593177, GSM: 99441688, CR No.: 178280', 0, 0, 'C');
